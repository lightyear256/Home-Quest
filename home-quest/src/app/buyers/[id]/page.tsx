...
const BuyerDetailsPage = () => {
  ...
  const getDisplayValue = (field, value) => {
    if (enumMappings[field] && enumMappings[field][value]) {
      return enumMappings[field][value];
    }
    if (enumMappings.Source[value]) {
      return enumMappings.Source[value];
    }
    return value;
  };
  ...
  return Object.entries(diff).map(([field, change]) => (
    ...
    <div key={field} className="mb-1">
      <span className="font-semibold capitalize text-slate-700">{field}:</span>
      {change && typeof change === "object" && "from" in change ? (
        <>
          <span className="text-red-500 line-through mx-2">
            {Array.isArray(change.from) ? change.from.join(", ") : change.from}
          </span>
